...
internal static class ChartExporter
{
    ...
    private static byte[] EncodePng(byte[] pixels, int width, int height)
    {
        ...
        // IDAT chunk — compress pixel data with deflate
        WriteChunk(ms, "IDAT", writer =>
        {
                ...
                deflateMs, System.IO.Compression.CompressionLevel.Fastest, leaveOpen: true))
            {
                ...
                {
                    deflate.WriteByte(0); // No filter for this scanline
                    deflate.Write(pixels, y * width * 4, width * 4);
                }
            }

            // zlib wrapper: header + deflate data + adler32 checksum
            byte[] deflateData = deflateMs.ToArray();

            // zlib header (CMF + FLG)
            writer.Write((byte)0x78); // CMF: deflate, window size 32K
            writer.Write((byte)0x01); // FLG: no dict, check bits

            writer.Write(deflateData);

            // Adler32 checksum of uncompressed data
            uint adler = ComputeAdler32(pixels, width, height);
            WriteBigEndian(writer, (int)adler);
        });
    ...
    private static void WriteChunk(MemoryStream ms, string type, Action<BinaryWriter> writeData)
    {
        ...
        // Length (big-endian)
        WriteBigEndianToStream(ms, data.Length);
        ...
        WriteBigEndianToStream(ms, (int)crc);
    }
